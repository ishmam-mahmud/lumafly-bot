...
class RolesCommand extends Command
{
  ...
  async run(msg: CommandoMessage, { catName }: RolesArgs): Promise<CommandoMessage | CommandoMessage[]>
  {
    try
    {
      if (catName.length < 3)
      ...
      const results = await getRepository(Category)
      ...
      const catsArr = results.sort((c1, c2) =>
      {
      ...
      if (catName === "*")
      {
        ...
        return await msg.embed(catEmbed);
      }
      ...
      const rolesArr = catAskedFor.roles.sort((r1, r2) =>
      {
